...
FOOTER_MARKERS = (
    "[保存]", "[Save]", "[Twitter]", "Google 学术搜索发送此邮件",
    "Google Scholar email alert", "列出快讯", "取消快讯",
)
...
def clean_title(title: str) -> str:
    ...
    title = RESOURCE_PREFIX_RE.sub("", title)
...
def extract_identifiers(title: str, metadata: str | None, snippet: str | None, url: str | None) -> dict:
    fields = [
        ("title", title), ("metadata_line", metadata or ""),
        ("snippet", snippet or ""), ("url", url or ""),
    ]
    values: dict[str, tuple[str | None, str | None]] = {
        "doi": (None, None), "arxiv_id": (None, None), "pmid": (None, None),
    }
    ...
    return {key: identifier_evidence(value, source) for key, (value, source) in values.items()}
...
def parse_metadata_line(metadata: str | None) -> tuple[list[dict], str | None, int | None]:
    ...
    for name in left.replace("…", "").split(",") if left else []:
        if name := name.strip():
            authors.append({"name": name, "orcid": None, "verification_status": "raw_email"})
...
def split_metadata_and_snippet(fragments: list[str]) -> tuple[str | None, str | None]:
    ...
    for fragment in fragments[:12]:
        ...
        if YEAR_RE.search(candidate) and " - " in candidate:
            return candidate, compact_snippet(fragments[consumed:])
...
def markdown_blocks(body: str) -> list[tuple[str, str, str | None, str | None]]:
    ...
    for index, match in enumerate(matches):
        ...
        blocks.append((clean_title(match.group("title")), match.group("href"), metadata, snippet))
    return blocks
...
def is_primary_result_anchor(anchor: Tag) -> bool:
    return bool(anchor.name == "a" and SCHOLAR_RESULT_URL.search(anchor_href(anchor)))


def is_inside(node: NavigableString, ancestor: Tag) -> bool:
    parent = node.parent
    return bool(parent is ancestor or (isinstance(parent, Tag) and ancestor in parent.parents))
...
def skip_html_text_node(node: NavigableString, title_anchor: Tag) -> bool:
    ...
    link_text = normalize_spaces(parent_anchor.get_text(" ", strip=True)).lower()
    if link_text in SHARE_TEXT or link_text in RESOURCE_TEXT:
        return True
    return any(marker in href for marker in SHARE_HREF_MARKERS)


def html_fragments_between(anchor: Tag, next_anchor: Tag | None) -> list[str]:
    fragments: list[str] = []
    for element in anchor.next_elements:
        if next_anchor is not None and element is next_anchor:
            break
        if isinstance(element, Tag) and element is not anchor and is_primary_result_anchor(element):
            break
...
def html_blocks(body: str) -> list[tuple[str, str, str | None, str | None]]:
    ...
    anchors = [anchor for anchor in soup.find_all("a", href=True) if is_primary_result_anchor(anchor)]
    blocks = []
    for index, anchor in enumerate(anchors):
        ...
        blocks.append((title, anchor_href(anchor), metadata, snippet))
    return blocks


def quality_warnings(title: str, metadata: str | None, primary_url: str | None) -> list[str]:
    warnings: list[str] = []
...
def make_candidate(*, context: SourceContext, position: int, title: str, href: str,
                   metadata: str | None, snippet: str | None, parser_strategy: str,
                   extracted_at: str, alert_name: str | None) -> dict:
    ...
    parse_state = "manual_review" if review_warnings else ("partial" if warnings else "complete")
    ...
    return {
        ...
        "source": {
            "source_type": "google_scholar_email", "message_id": context.message_id,
            "thread_id": context.thread_id, "received_at": context.received_at,
            "sender": context.sender, "subject": context.subject, "alert_name": alert_name,
        ...
        "parse_status": {"state": parse_state, "warnings": warnings, "errors": [],
...
def parse_alert_body(body: str, context: SourceContext, *, content_type: str = "auto",
                     extracted_at: str | None = None) -> list[dict]:
    ...
    return [
        make_candidate(
            context=context, position=index, title=title, href=href, metadata=metadata,
            snippet=snippet, parser_strategy=strategy, extracted_at=extracted_at,
...
def main() -> int:
    ...
    context = SourceContext(
        message_id=args.message_id, received_at=args.received_at,
        sender=args.sender, subject=args.subject,
    )
    records = parse_alert_body(
        args.input.read_text(encoding="utf-8"), context, content_type=args.content_type,
    )
